...
function findPropsForDefineMessages(node) {
    var messages = [];
...
function findPropsForFormatMessage(node) {
    var message = {};
    node.properties.forEach(function (p) {
        if (ts.isPropertyAssignment(p) &&
...
function findProps(node, methodName) {
    var res = [];
    function find(node) {
        if (!node) {
            return undefined;
        ...
        if (ts.isObjectLiteralExpression(node)) {
            if (methodName === "defineMessages") {
                res.push.apply(res, findPropsForDefineMessages(node));
            }
            else if (methodName === "formatMessage") {
                res.push.apply(res, findPropsForFormatMessage(node));
            }
            else {
...
function findMethodCallsWithName(sourceFile, methodName) {
    var res = [];
    forAllVarDecls(sourceFile, function (el) {
        if (isMethodCall(el, methodName)) {
            if (ts.isCallExpression(el.initializer) &&
                ...
                var props = findProps(nodeProps, methodName);
...
function main(contents) {
    ...
    var dm = findMethodCallsWithName(sourceFile, "defineMessages");
    var fm = findMethodCallsWithName(sourceFile, "formatMessage");
    var res = elements
