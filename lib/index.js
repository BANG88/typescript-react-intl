...
function isDefineMessages(el, tagName) {
    return (el.kind === ts.SyntaxKind.VariableDeclaration &&
        el.initializer &&
...
function findFirstJsxOpeningLikeElementWithName(node, tagName, dm) {
    ...
    function find(node) {
        ...
        if (dm && node.getNamedDeclarations) {
            ...
            nd.forEach(function (element) {
                element.forEach(function (el) {
                    if (isDefineMessages(el, tagName)) {
                        if (el.initializer.kind === ts.SyntaxKind.CallExpression &&
...
function main(contents) {
    ...
    var emptyObject = function (o) { return JSON.stringify(o) === "{}"; };
    var res = elements
